...
8. **次に何をやるかを提案する場合、および方向性の判断を扱う場合は [ROADMAP.md](../ROADMAP.md) を読むこと。** 全リポジトリ共通の1枚。「やらない / 保留」に記録済みの内容を再提案しないこと。セッション中に方向性の判断が出た場合は、その場で追記を提案すること。着手済みの作業単位はここではなく各リポジトリのGitHub Issuesにある。無関係な小規模修正では読まなくてよい。
