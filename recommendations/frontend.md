...
- **前提**: 日時の正となる判定はサーバー側で行う（[Architecture Standard「時刻・タイムゾーン」](../standards/architecture/README.md#時刻タイムゾーン)）。期限判定、「今日」の決定、日付の集計をクライアントで再計算しない。端末のタイムゾーンと時計に依存するため、サーバーの判定と静かにズレる。
- 上記を踏まえたうえで、クライアント側に残る用途（入力補助、カレンダーの隣接日、相対表示等）に利用する。
- プレーンな `Date` を入出力する純関数の集まり。関数単位のimportでtree-shakeが効き、値を破壊的に変更しない。
- `shadcn/ui` Calendarの内部エンジンであるReact DayPickerがdate-fnsに依存するため、別の日付ライブラリを足さず一本化できる。
- タイムゾーンを明示した表示が必要な場合は `@date-fns/tz` の `TZDate` を併用する（1.5.0 / 2026-05-21）。
- 確認日時点で最終リリース 4.4.0 (2026-05-29)、MIT。
